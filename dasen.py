...
parser.add_argument("--method", type=str, default='DASEN',
                    choices=['DASEN'])

parser.add_argument("--init_method", type=str, default='rand',
                    choices=['rand', 'trunc_normal', 'ones', 'dirac', 'xavier_uniform', 'xavier_normal', 'kaiming_uniform', 'kaiming_normal'])
...
parser.add_argument("--num_sample", type=int, default=50,
        help="number of p samples without learnable alpha (default: 50)")
...
save_path = args.save_path + f'/{args.target_order}-{args.seed}/{args.method}-{args.init_method}'
...
if args.ignore_wandb == False:
    ...
    run_name = f"{args.method}-order{args.target_order}-seed{args.seed}-fold{args.fold}-lr{args.lr_init}-wd{args.wd}-epc{args.epochs}-tol{args.tol}-init{args.init_method}"
    if args.rm_feature != -999:
...
if args.only_gnn:
    ...
    if args.init_method == 'rand':
        alpha = nn.Parameter(torch.rand(6))
    if args.init_method == 'trunc_normal':
        alpha = nn.init.trunc_normal_(nn.Parameter(torch.empty(6), requires_grad=True), a=1e-10)
    if args.init_method == 'ones':
        alpha = nn.init.ones_(nn.Parameter(torch.empty(6), requires_grad=True))

    dir_dist = dist.Dirichlet(alpha)
## -----------------------------------------------------------------------------
...
optimizer = torch.optim.Adam([alpha], lr=args.lr_init, weight_decay=args.wd)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs, eta_min=1e-4)

### ---------------------------------------------------------------------------------------------
### In case of Learnable Alpha of Dirichlet Distribution (DASEN)
### ---------------------------------------------------------------------------------------------
## Training
...
for epoch in range(1, args.epochs + 1):
    ...
    ##-------------------------------------------------------------------------
    ### Forward
    alpha.data = alpha.data.clamp_min(1e-10)
    tr_w = dir_dist.rsample()

    if not args.only_gnn:
        tr_ens_pred = tr_w[0] * torch.tensor(tr_df['lr_pred'])
        tr_ens_pred += tr_w[1] * torch.tensor(tr_df['rr_pred'])
        tr_ens_pred += tr_w[2] * torch.tensor(tr_df['mlp_pred'])
        tr_ens_pred += tr_w[3] * torch.tensor(tr_df['gat_pred'])
        tr_ens_pred += tr_w[4] * torch.tensor(tr_df['gcn_pred'])
        tr_ens_pred += tr_w[5] * torch.tensor(tr_df['mag_pred'])
    else:
        tr_ens_pred = tr_w[0] * torch.tensor(tr_df['gat_pred']) 
        tr_ens_pred += tr_w[1] * torch.tensor(tr_df['gcn_pred'])
        tr_ens_pred += tr_w[2] * torch.tensor(tr_df['mag_pred'])
    
    tr_df['ens_pred'] = tr_ens_pred.detach().numpy()
    tr_gt = np.array(tr_df['tr_gt'].tolist())    
    ...
    ##-------------------------------------------------------------------------
    ### validation loss
    val_w = dir_dist.rsample([30])
    val_group_loss = 999999
    for i in range(len(val_w)):
        if not args.only_gnn:
            val_ens_pred = val_w[i, 0] * torch.tensor(val_df['lr_pred'])
            val_ens_pred += val_w[i, 1] * torch.tensor(val_df['rr_pred'])
            val_ens_pred += val_w[i, 2] * torch.tensor(val_df['mlp_pred'])
            val_ens_pred += val_w[i, 3] * torch.tensor(val_df['gat_pred'])
            val_ens_pred += val_w[i, 4] * torch.tensor(val_df['gcn_pred'])
            val_ens_pred += val_w[i, 5] * torch.tensor(val_df['mag_pred'])
        else:
            val_ens_pred += val_w[i, 0] * torch.tensor(val_df['gat_pred'])
            val_ens_pred += val_w[i, 1] * torch.tensor(val_df['gcn_pred'])
            val_ens_pred += val_w[i, 2] * torch.tensor(val_df['mag_pred'])

        val_df['ens_pred'] = val_ens_pred.detach().numpy()
        val_gt = np.array(val_df['val_gt'].tolist())
        
        val_avg_loss_tmp = 0
        for idx in val_df['graph_idx'].unique():
            idx_ = val_df[val_df['graph_idx'] == idx].index
            out = val_ens_pred[idx_]
            gt = torch.tensor(val_gt[idx_])
            val_loss = eval_criterion(out, gt)
            val_avg_loss_tmp += val_loss
        val_avg_loss_tmp /= len(val_df)            
    
        val_group_loss_dict, val_group_loss_tmp, _, _ = utils.cal_group_loss(val_gt, val_df['ens_pred'])        
        val_worst_loss_tmp = max(val_group_loss_dict.values())
        
        if val_group_loss_tmp < val_group_loss:
            te_w = val_w[i]
        
            val_avg_loss = val_avg_loss_tmp
            val_group_loss = val_group_loss_tmp
            val_worst_loss = val_worst_loss_tmp                        
                       
    ## ----------------------------------------------------------------------------
    ### test loss
    if not args.only_gnn:
        te_ens_pred = te_w[0] * torch.tensor(te_df['lr_pred'])
        te_ens_pred += te_w[1] * torch.tensor(te_df['rr_pred'])
        te_ens_pred += te_w[2] * torch.tensor(te_df['mlp_pred'])
        te_ens_pred += te_w[3] * torch.tensor(te_df['gat_pred'])
        te_ens_pred += te_w[4] * torch.tensor(te_df['gcn_pred'])
        te_ens_pred += te_w[5] * torch.tensor(te_df['mag_pred'])
    else:
        te_ens_pred = te_w[0] * torch.tensor(te_df['gat_pred'])
        te_ens_pred += te_w[1] * torch.tensor(te_df['gcn_pred'])
        te_ens_pred += te_w[2] * torch.tensor(te_df['mag_pred'])
